...
namespace Core.ObjectMap
{
	...
	public class ObjectMap : BaseUnityPlugin
	{
		...
		internal static readonly Dictionary<GameObject, ObjectCtrlInfo> MapObjects =
			new Dictionary<GameObject, ObjectCtrlInfo>();

		internal static readonly Dictionary<Transform, IObjectProperties> OriginalProperties =
			new Dictionary<Transform, IObjectProperties>();

		private void Awake()
		{
			Logger = base.Logger;

			_objectLimit = Config.Bind("General", "Object Limit", 3000,
				"Maps with more objects than this number will be ignored in order to preserve performance.");

			LoadLights = Config.Bind("General", "Manage Lights", true,
				"In some cases this can conflict with other light managers such as GraphicsMod and should be turned off.");
			...
			StudioAPI.StudioLoadedChanged += (sender, args) =>
			{
				...
				CustomToolbarButtons.AddLeftToolbarButton(tex2D, ResetTransformOfSelected);
		...
		{
			if (Singleton<Studio.Studio>.Instance.treeNodeCtrl.selectNodes.Length <= 0) return;

			var selectNodes = Singleton<Studio.Studio>.Instance.treeNodeCtrl.selectNodes;
			foreach (var treeNodeObject in selectNodes)
			{
				if (!Singleton<Studio.Studio>.Instance.dicInfo.TryGetValue(treeNodeObject, out var objectCtrlInfo))
					return;
				...
				if (!OriginalProperties.TryGetValue(target, out var originalProperties)) return;
		...
		private IEnumerator CoroutineCaptureMapChange()
		{
			...
			while (Studio.Map.Instance.isLoading) yield return new WaitForEndOfFrame();

			//yield return new WaitUntil(() => Studio.Map.Instance.isLoading == false);
			//yield return new WaitForEndOfFrame();

			if (!TryGetMapRootObjects(out var mapRootObjects)) yield break;

			if (IsObjectAboveLimit(mapRootObjects)) yield break;
		...
		private static bool IsObjectAboveLimit(GameObject[] mapRootObjects)
		{
			...
			{
				Logger.LogMessage(
					$"Skipped processing map because it has more objects than the configured limit ({childCount} / {_objectLimit.Value}).");
				return true;
			}
		...
		private IEnumerator ToggleActiveForOneFrame(GameObject rootObject)
		{
			var inactiveChildren =
				rootObject
					...
					.Where(gameObj => !gameObj.activeSelf)
					.ToArray();
