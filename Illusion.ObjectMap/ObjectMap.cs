...
namespace Core.ObjectMap
{
	...
	public class ObjectMap : BaseUnityPlugin
	{
		...
		private static ConfigEntry<bool> _loadLights;
		...
		private void Awake()
		{
			...
			_loadLights = Config.Bind("General", "Manage Lights", true, "In some cases this can conflict with other light managers such as GraphicsMod and should be turned off.");
		...
		private IEnumerator CoroutineCaptureMapChange()
		{
...
#if KK
            var sceneData = Scene.Instance._baseScene;
#else
            var sceneData = Scene._baseScene;
#endif

			var scene = Scene.GetScene(sceneData.levelName);
            var mapRootObjects = scene.GetRootGameObjects();

            if (mapRootObjects.Length <= 0)
			{
				yield break;
			}
            ...
            {
                RecursiveCreateNode(mapRootObject, null);

                Studio.Studio.instance.treeNodeCtrl.RefreshHierachy();
                yield return EnableDisabledForAFrame(mapRootObject);
