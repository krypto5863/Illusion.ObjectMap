...
namespace Core.ObjectMap
{
	public static class StudioObjectFactory
	{
		public static void SetupStudioObjectRecursively(GameObject gameObj, ObjectCtrlInfo parent)
		{
			...
			if (!CreateBasedOnComponents(gameObj, objectIndex, out var newItem)) return;
			...
			foreach (var child in gameObj.Children()) SetupStudioObjectRecursively(child, newItem);
		...
		private static int GetObjectIndex(GameObject gameObj)
		{
			...
			if (occupied)
			{
				...
				ObjectMap.Logger.LogMessage(
					$"Index {hash} for {gameObj.GetPathWithSiblingIndex()} was already occupied! Using {newIndex} instead!");
				hash = newIndex;
			}
		...
		private static OCIItem CreateItemFromGameObject(GameObject gameObj, int hash)
		{
			...
			if (renderer == null)
			{
				ObjectMap.Logger.LogError($"Could not find renderer for {gameObj.name}");
				return null;
			}
			...
			ObjectMap.OriginalProperties[gameObj.transform] = new ItemProperties
			{
				...
				{
					gameObj.transform.localPosition, gameObj.transform.localEulerAngles, gameObj.transform.lossyScale
				}
			};
		...
		private static OCILight CreateLightFromGameObject(GameObject gameObj, int hash)
		{
			...
			else if (isCharaLayerActive) newLight.lightTarget = Info.LightLoadInfo.Target.Chara;
